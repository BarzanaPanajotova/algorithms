 ...
 * 
 * int solution(int A[], int N); that, given a non-empty zero-indexed array A of
 * N integers, returns the minimal difference that can be achieved.
 * 
 * For example, given:
 * 
 ...
 *
 */
public final class NumbersOnTape {

	private NumbersOnTape() {
	}

	/**
	 * Basically traversing the array from left to right keeping the minimal
	 * difference found until now and calculating the new one as: <br>
	 * The sum of elements is always constant. If we keep the left sum and
	 * subtract it twice we effectively calculate the difference between right
	 * and left. <br>
	 * <br>
	 * diff = |sum - 2*left| = |(sum - left) - left| = |right - left|
	 * 
	 * @param tape
	 *            The input array which is with [2..100,000] number of elements.
	 *            Every element is in range [−1,000..1,000].
	 * @return The smallest difference between left and right sum of elements.
	 */
	public static int getSmallestDiff(int[] tape) {
		if (tape.length < 2 || tape.length > 100000) {
			throw new IllegalArgumentException("Input array is with illegal length!");
		}

		int sum = Arrays.stream(tape).sum();
		int minDiff = Integer.MAX_VALUE;

		int left = 0;
		for (int i = 0; i < tape.length - 1; i++) {
			left += tape[i];
			minDiff = Math.min(minDiff, Math.abs(sum - 2 * left));
		}
